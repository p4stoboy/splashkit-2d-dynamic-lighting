...
void update_player(Player& player, OpenCLWrapper& openclWrapper) {
    ...
    // Movement (independent of heading)
    ...
    if (new_x >= 0 && new_x < gridWidth && new_y >= 0 && new_y < gridHeight) {
        ...
        if (hit_point.x < 0) {  // No collision
            ...
            // If collision, stop the player's velocity in that direction
...
void render_player(const Player& player) {
    ...
    // Calculate perpendicular direction for foot placement
    ...
    // Foot animation
    double time = player.position.x + player.position.y; // Use position as a time variable
    double animation_speed = 0.6; // Adjust this to change animation speed
    double foot_offset = sin(time * animation_speed) * body_height * 0.8; // Increased range of motion

    // Calculate foot positions
    double foot_side_offset = body_width * 0.3; // Distance from center line
    ...
    double foot2_x = screen_x - perp_x * foot_side_offset - cos_heading * foot_offset; // Note the negative offset
    double foot2_y = screen_y - perp_y * foot_side_offset - sin_heading * foot_offset;

    // Function to draw rotated rectangle (foot)
    ...
    // Draw a line indicating player's heading (optional, for debugging)
